    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? '起動中...' : '結果レポートの生成を開始' }}
...
const doStartSimulation = async () => {
  ...
  startError.value = null
  addLog('デュアルプラットフォーム並行シミュレーションを起動中...')
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(`最大シミュレーション回数を設定: ${props.maxRounds}`)
    }

    addLog('動的グラフ更新モードを有効化しました')
    
    const res = await startSimulation(params)
    
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog('✓ 古いシミュレーションログをクリアし、再開しました')
      }
      addLog('✓ シミュレーションエンジンの起動に成功')
      ...
      startError.value = res.error || '起動失敗'
      addLog(`✗ 起動失敗: ${res.error || '不明なエラー'}`)
      emit('update-status', 'error')
    }
  } catch (err) {
    startError.value = err.message
    addLog(`✗ 起動例外: ${err.message}`)
    emit('update-status', 'error')
  } finally {
    isStarting.value = false
  }
...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(`停止失敗: ${res.error || '不明なエラー'}`)
    }
  } catch (err) {
    addLog(`停止例外: ${err.message}`)
  } finally {
    isStopping.value = false
  }
}
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (isCompleted || platformsCompleted) {
        if (platformsCompleted && !isCompleted) {
          addLog('✓ 全プラットフォームのシミュレーション終了を検出')
        }
        addLog('✓ シミュレーション完了')
        phase.value = 2
        stopPolling()
    ...
    console.warn('実行ステータスの取得に失敗:', err)
  }
}
...
const fetchRunStatusDetail = async () => {
  ...
  try {
    ...
    console.warn('詳細ステータスの取得に失敗:', err)
  }
}
...
const handleNextStep = async () => {
  ...
  if (isGeneratingReport.value) {
    addLog('レポート生成リクエスト送信済み、お待ちください...')
    return
  }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✓ レポート生成タスクを開始: ${reportId}`)
      
      // 跳转到报告页面
      router.push({ name: 'Report', params: { reportId } })
    } else {
      addLog(`✗ レポート生成の起動に失敗: ${res.error || '不明なエラー'}`)
      isGeneratingReport.value = false
    }
  } catch (err) {
    addLog(`✗ レポート生成の起動で例外: ${err.message}`)
    isGeneratingReport.value = false
  }
}
