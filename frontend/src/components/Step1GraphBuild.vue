    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            LLMがドキュメント内容とシミュレーション要件を分析し、現実シードを抽出して適切なオントロジー構造を自動生成します
          </p>
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            生成されたオントロジーに基づき、ドキュメントを自動分割してZepでナレッジグラフを構築し、エンティティと関係を抽出、時系列記憶とコミュニティサマリーを生成します
          </p>
          ...
          <div class="stats-grid">
            ...
            <div class="stat-card">
              ...
              <span class="stat-label">関係エッジ</span>
            </div>
            <div class="stat-card">
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 2, 'completed': currentPhase >= 2 }">
        ...
        <div class="card-content">
          ...
          <p class="description">グラフ構築が完了しました。次のステップでシミュレーション環境を構築してください</p>
          <button 
            class="action-btn" 
          ...
          >
            ...
            {{ creatingSimulation ? '作成中...' : '環境構築へ進む ➝' }}
          </button>
        </div>
      </div>
    </div>
...
const handleEnterEnvSetup = async () => {
  if (!props.projectData?.project_id || !props.projectData?.graph_id) {
    console.error('プロジェクトまたはグラフ情報がありません')
    return
  }
  ...
  try {
    ...
    if (res.success && res.data?.simulation_id) {
      ...
      console.error('シミュレーション作成失敗:', res.error)
      alert('シミュレーション作成失敗: ' + (res.error || '不明なエラー'))
    }
  } catch (err) {
    console.error('シミュレーション作成例外:', err)
    alert('シミュレーション作成例外: ' + err.message)
  } finally {
