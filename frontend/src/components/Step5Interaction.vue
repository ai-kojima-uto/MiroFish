            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-else-if="currentSectionIndex === idx + 1" class="loading-state">
                  ...
                  <span class="loading-text">{{ section.title }}を生成中...</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="action-bar-tabs">
            ...
            <div class="agent-dropdown" v-if="profiles.length > 0">
              ...
              >
                ...
                <span>{{ selectedAgent ? selectedAgent.username : '世界の任意の個体と対話' }}</span>
                <svg class="dropdown-arrow" :class="{ open: showAgentDropdown }" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2">
                  <polyline points="6 9 12 15 18 9"></polyline>
                </svg>
              </button>
              <div v-if="showAgentDropdown" class="dropdown-menu">
                <div class="dropdown-header">対話相手を選択</div>
                <div 
                ...
                >
                  ...
                  <div class="agent-info">
                    ...
                    <span class="agent-role">{{ agent.profession || '不明な職業' }}</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div v-if="activeTab === 'chat'" class="chat-container">
          ...
          <div v-if="chatTarget === 'report_agent'" class="report-agent-tools-card">
            <div class="tools-card-header">
              ...
              <div class="tools-card-info">
                ...
                <div class="tools-card-subtitle">レポート生成エージェントの高速対話バージョン。4種類の専門ツールを呼び出せ、MiroFishの完全なメモリを保持</div>
              </div>
              ...
              <div class="tools-grid">
                <div class="tool-item tool-purple">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">InsightForge 深度帰因</div>
                    <div class="tool-desc">現実世界のシードデータとシミュレーション環境の状態を照合し、Global/Local Memoryメカニズムを組み合わせて時空を超えた深度帰因分析を提供</div>
                  </div>
                </div>
                <div class="tool-item tool-blue">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">PanoramaSearch 全景追跡</div>
                    <div class="tool-desc">グラフ構造に基づく幅優先探索アルゴリズムで事象の伝播経路を再構築し、情報フロー全体のトポロジー構造を捕捉</div>
                  </div>
                </div>
                <div class="tool-item tool-orange">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">QuickSearch 高速検索</div>
                    <div class="tool-desc">GraphRAGベースのリアルタイムクエリインターフェース。インデックス効率を最適化し、ノード属性や離散的な事実の高速抽出に使用</div>
                  </div>
                </div>
                <div class="tool-item tool-green">
                  ...
                  <div class="tool-content">
                    <div class="tool-name">InterviewSubAgent 仮想インタビュー</div>
                    <div class="tool-desc">自律型インタビュー。シミュレーション世界の個体と並行して多ターンの対話を行い、非構造化の意見データと心理状態を収集</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="chatTarget === 'agent' && selectedAgent" class="agent-profile-card">
            <div class="profile-card-header">
              ...
              <div class="profile-card-info">
                ...
                <div class="profile-card-meta">
                  ...
                  <span class="profile-card-profession">{{ selectedAgent.profession || '不明な職業' }}</span>
                </div>
              </div>
            ...
            <div v-if="showFullProfile && selectedAgent.bio" class="profile-card-body">
              <div class="profile-card-bio">
                <div class="profile-card-label">プロフィール</div>
          ...
          <div class="chat-messages" ref="chatMessages">
            <div v-if="chatHistory.length === 0" class="chat-empty">
              ...
              <p class="empty-text">
                {{ chatTarget === 'report_agent' ? 'Report Agent と対話して、レポートの内容を深く理解しましょう' : 'シミュレーション個体と対話して、彼らの意見を聞いてみましょう' }}
              </p>
            </div>
            <div 
        ...
        <div v-if="activeTab === 'survey'" class="survey-container">
          ...
          <div class="survey-setup">
            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">調査対象を選択</span>
                <span class="selection-count">選択済み {{ selectedAgents.size }} / {{ profiles.length }}</span>
              </div>
              <div class="agents-grid">
                ...
                >
                  ...
                  <div class="checkbox-info">
                    ...
                    <span class="checkbox-role">{{ agent.profession || '不明な職業' }}</span>
                  </div>
              ...
              <div class="selection-actions">
                <button class="action-link" @click="selectAllAgents">すべて選択</button>
                <span class="action-divider">|</span>
                <button class="action-link" @click="clearAgentSelection">クリア</button>
              </div>
            </div>

            <div class="setup-section">
              <div class="section-header">
                <span class="section-title">アンケートの質問</span>
              </div>
              <textarea 
                v-model="surveyQuestion"
                class="survey-input"
                placeholder="選択したすべての対象に聞きたい質問を入力してください..."
                rows="3"
              ></textarea>
            </div>
          ...
          <div v-if="surveyResults.length > 0" class="survey-results">
            <div class="results-header">
              <span class="results-title">調査結果</span>
              <span class="results-count">{{ surveyResults.length }} 件の回答</span>
            </div>
              ...
              >
                <div class="result-header">
                  ...
                  <div class="result-info">
                    ...
                    <span class="result-role">{{ result.profession || '不明な職業' }}</span>
                  </div>
                </div>
...
const sendToReportAgent = async (message) => {
  addLog(`Report Agent へ送信: ${message.substring(0, 50)}...`)
  ...
  if (res.success && res.data) {
    chatHistory.value.push({
      role: 'assistant',
      content: res.data.response || res.data.answer || '応答なし',
      timestamp: new Date().toISOString()
    })
    addLog('Report Agent が返信しました')
  } else {
    throw new Error(res.error || 'リクエストに失敗しました')
  }
}

const sendToAgent = async (message) => {
  if (!selectedAgent.value || selectedAgentIndex.value === null) {
    throw new Error('最初にシミュレーション個体を選択してください')
  }
  
  addLog(`${selectedAgent.value.username} へ送信: ${message.substring(0, 50)}...`)
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .slice(-6)
      .map(msg => `${msg.role === 'user' ? '質問者' : 'あなた'}：${msg.content}`)
      .join('\n')
    prompt = `以下は以前の会話です：\n${historyContext}\n\n新しい質問：${message}`
  }
  ...
  if (res.success && res.data) {
    ...
    if (responseContent) {
      ...
      addLog(`${selectedAgent.value.username} が返信しました`)
    } else {
      throw new Error('応答データがありません')
    }
  } else {
    throw new Error(res.error || 'リクエストに失敗しました')
  }
...
const submitSurvey = async () => {
  ...
  addLog(`${selectedAgents.value.size} 件の対象にアンケートを送信中...`)
    ...
    if (res.success && res.data) {
      ...
      for (const interview of interviews) {
        ...
        if (typeof resultsDict === 'object' && !Array.isArray(resultsDict)) {
          ...
          if (agentResult) {
            responseContent = agentResult.response || agentResult.answer || '応答なし'
          }
        } else if (Array.isArray(resultsDict)) {
          // 兼容数组格式
          const matchedResult = resultsDict.find(r => r.agent_id === agentIdx)
          if (matchedResult) {
            responseContent = matchedResult.response || matchedResult.answer || '応答なし'
          }
        }
      ...
      addLog(`${surveyResults.value.length} 件の回答を受信しました`)
    } else {
      throw new Error(res.error || 'リクエストに失敗しました')
    }
  } catch (err) {
    addLog(`アンケートの送信に失敗しました: ${err.message}`)
  } finally {
...
const loadReportData = async () => {
  ...
  try {
    addLog(`レポートデータを読み込み中: ${props.reportId}`)
    ...
    addLog(`レポートの読み込みに失敗しました: ${err.message}`)
  }
}
...
const loadAgentLogs = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog('レポートデータの読み込み完了')
    }
  } catch (err) {
    addLog(`レポートログの読み込みに失敗しました: ${err.message}`)
  }
}
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`${profiles.value.length} 件のシミュレーション個体を読み込みました`)
    }
  } catch (err) {
    addLog(`シミュレーション個体の読み込みに失敗しました: ${err.message}`)
  }
}
