<template>
  ...
  >
    ...
    <div class="section-header">
      ...
      <span class="section-title">シミュレーション履歴</span>
      <div class="section-line"></div>
    </div>
        ...
        <div class="card-files-wrapper">
          ...
          <div class="files-list" v-if="project.files && project.files.length > 0">
            ...
            <div v-if="project.files.length > 3" class="files-more">
              +{{ project.files.length - 3 }} ファイル
            </div>
          </div>
          <!-- 无文件时的占位 -->
          <div class="files-empty" v-else>
            <span class="empty-file-icon">◇</span>
            <span class="empty-file-text">ファイルなし</span>
          </div>
        </div>
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="modal-section">
                <div class="modal-label">シミュレーション要件</div>
                <div class="modal-requirement">{{ selectedProject.simulation_requirement || 'なし' }}</div>
              </div>

              <!-- 文件列表 -->
              <div class="modal-section">
                <div class="modal-label">関連ファイル</div>
                ...
                <div class="modal-empty" v-else>関連ファイルなし</div>
              </div>
            </div>
            ...
            <div class="modal-divider">
              ...
              <span class="divider-text">シミュレーション再生</span>
              <span class="divider-line"></span>
            </div>
            ...
            <div class="modal-actions">
              ...
              >
                ...
                <span class="btn-text">グラフ構築</span>
              </button>
              <button 
              ...
              >
                ...
                <span class="btn-icon">◆</span>
                <span class="btn-text">分析レポート</span>
              </button>
            </div>
            <!-- 不可回放提示 -->
            <div class="modal-playback-hint">
              <span class="hint-text">Step3「シミュレーション開始」とStep5「インタラクション」は実行中のみ利用可能で、履歴からの再生はサポートされていません</span>
            </div>
          </div>
        </div>
      </Transition>
...
const formatRounds = (simulation) => {
  const current = simulation.current_round || 0
  const total = simulation.total_rounds || 0
  if (total === 0) return '未開始'
  return `${current}/${total} ラウンド`
}
...
const loadHistory = async () => {
  try {
    ...
    console.error('履歴プロジェクトの読込に失敗:', error)
    projects.value = []
  } finally {
    loading.value = false
  }
