    ...
    <header class="app-header">
      ...
      <div class="header-right">
        <div class="workflow-step">
          ...
          <span class="step-name">環境構築</span>
        </div>
...
const handleNextStep = (params = {}) => {
  addLog('Step 3 に進む: シミュレーション開始')

  // 记录模拟轮数配置
  if (params.maxRounds) {
    addLog(`カスタムシミュレーション回数: ${params.maxRounds} 回`)
  } else {
    addLog('自動設定のシミュレーション回数を使用')
  }
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        if (closeRes.success) {
          ...
          addLog(`シミュレーション環境の停止に失敗: ${closeRes.error || '未知错误'}`)
          // 如果优雅关闭失败，尝试强制停止
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(`シミュレーション環境の停止で例外発生: ${closeErr.message}`)
        // 如果优雅关闭异常，尝试强制停止
        await forceStopSimulation()
      }
    } else {
      ...
      if (simRes.success && simRes.data?.status === 'running') {
        addLog('シミュレーション実行中を検出、停止中...')
        await forceStopSimulation()
      }
    }
  } catch (err) {
    // 检查环境状态失败不影响后续流程
    console.warn('シミュレーション状態確認失敗:', err)
  }
}
...
const forceStopSimulation = async () => {
  try {
    ...
    if (stopRes.success) {
      ...
      addLog(`強制停止に失敗: ${stopRes.error || '未知错误'}`)
    }
  } catch (err) {
    addLog(`強制停止で例外発生: ${err.message}`)
  }
}
