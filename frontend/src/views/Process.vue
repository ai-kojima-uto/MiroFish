    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        <div class="panel-header">
          <div class="header-left">
            ...
            <span class="header-title">リアルタイムナレッジグラフ</span>
          </div>
          <div class="header-right">
            ...
            <div class="action-buttons">
                ...
                <button class="action-btn" @click="toggleFullScreen" :title="isFullScreen ? '全画面を終了' : '全画面表示'">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="graphLoading" class="graph-loading">
            ...
            <p class="loading-text">グラフデータを読み込み中...</p>
          </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-text">オントロジー生成を待機中</p>
            <p class="waiting-hint">生成完了後、自動的にグラフ構築を開始します</p>
          </div>
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-text">グラフ構築中</p>
            <p class="waiting-hint">データをまもなく表示します...</p>
          </div>
      ...
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
        <div class="panel-header dark-header">
          ...
          <span class="header-title">構築フロー</span>
        </div>
        ...
        <div class="process-content">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">オントロジー生成</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">インターフェース説明</div>
                <div class="detail-content">
                  ドキュメントをアップロードすると、LLMがドキュメントの内容を分析し、世論シミュレーションに適したオントロジー構造（エンティティタイプ＋関係タイプ）を自動生成します
                </div>
              </div>
              
              <!-- 本体生成进度 -->
              <div class="detail-section" v-if="ontologyProgress && currentPhase === 0">
                <div class="detail-label">生成進捗</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">生成されたエンティティタイプ ({{ projectData.ontology.entity_types?.length || 0 }})</div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">生成された関係タイプ ({{ projectData.ontology.relation_types?.length || 0 }})</div>
                <div class="relation-list">
                  ...
                  <div v-if="(projectData.ontology.relation_types?.length || 0) > 5" class="relation-more">
                    +{{ projectData.ontology.relation_types.length - 5 }} 件の関係...
                  </div>
                </div>
              </div>
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">オントロジー生成を待機中...</div>
              </div>
            </div>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">グラフ構築</div>
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">インターフェース説明</div>
                <div class="detail-content">
                  生成されたオントロジーを基に、ドキュメントをチャンクに分割し Zep API を呼び出して知識グラフを構築し、エンティティと関係を抽出します
                </div>
              </div>
              
              <!-- 等待本体完成 -->
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">オントロジー生成の完了を待機中...</div>
              </div>
              
              <!-- 构建进度 -->
              <div class="detail-section" v-if="buildProgress && currentPhase >= 1">
                <div class="detail-label">構築進捗</div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">構築完了</div>
                <div class="phase-api">次のステップへ進む準備完了</div>
              </div>
...
const handleNewProject = async () => {
  ...
  try {
    ...
    ontologyProgress.value = { message: 'ファイルをアップロードしてドキュメントを分析中...' }
    ...
    if (response.success) {
      ...
      error.value = response.error || 'オントロジー生成に失敗しました'
    }
  } catch (err) {
    console.error('Handle new project error:', err)
    error.value = 'プロジェクトの初期化に失敗しました: ' + (err.message || '不明なエラー')
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || 'プロジェクトの読み込みに失敗しました'
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = 'プロジェクトの読み込みに失敗しました: ' + (err.message || '不明なエラー')
  } finally {
    loading.value = false
  }
}
...
const updatePhaseByStatus = (status) => {
  switch (status) {
    ...
    case 'graph_completed':
      ...
      break
    case 'failed':
      error.value = projectData.value?.error || '処理に失敗しました'
      break
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || 'グラフ構築の起動に失敗しました'
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = 'グラフ構築の起動に失敗しました: ' + (err.message || '不明なエラー')
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        stopPolling()
        stopGraphPolling()
        error.value = 'グラフ構築に失敗しました: ' + (task.error || '不明なエラー')
        buildProgress.value = null
      }
    }
  } catch (err) {
